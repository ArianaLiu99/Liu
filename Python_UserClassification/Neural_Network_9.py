...
table = data.sheets()[8]            #Open the next worksheet
...
new_worksheet = new_workbook.get_sheet(8)   #Open the current sheet

data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[7]       #Get the previous sheet
...
for i in range (0,543):
    if int(test_label[i])==result[i]:
        accu9+=1
acc_rate9=accu9/543
print("The prediction accuracy rate of the nineth month is ", acc_rate9)


#Adding result to the data
old_workbook = xlrd.open_workbook('data_sep.xls')
new_workbook = copy(old_workbook)
new_worksheet = new_workbook.get_sheet(8)  #Get the current sheet
data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[8]   #Open the current sheet
