...
table = data.sheets()[10]            #Open the next worksheet
...
new_worksheet = new_workbook.get_sheet(10)   #Open the current sheet

data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[9]       #Get the previous sheet
...
for i in range (0,543):
    if int(test_label[i])==result[i]:
        accu11+=1
acc_rate11=accu11/543
print("The prediction accuracy rate of the eleventh month is ", acc_rate11)


#Adding result to the data
old_workbook = xlrd.open_workbook('data_sep.xls')
new_workbook = copy(old_workbook)
new_worksheet = new_workbook.get_sheet(10)  #Get the current sheet
data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[10]   #Open the current sheet
