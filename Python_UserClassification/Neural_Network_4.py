...
for i in range (0,543):
    if int(test_label[i])==result[i]:
        accu4+=1
acc_rate4=accu4/544
print("The prediction accuracy rate of the forth month is ", acc_rate4)
...
old_workbook = xlrd.open_workbook('data_sep.xls')
new_workbook = copy(old_workbook)
new_worksheet = new_workbook.get_sheet(3)  #Get the current sheet
data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[3]   #Open the current sheet
