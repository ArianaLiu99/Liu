...
table = data.sheets()[5]            #Open the next worksheet
...
new_worksheet = new_workbook.get_sheet(5)   #Open the current sheet

data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[4]       #Get the previous sheet
...
for i in range (0,542):
    if int(test_label[i])==result[i]:
        accu6+=1
acc_rate6=accu6/542
print("The prediction accuracy rate of the sixth month is ", acc_rate6)
...
old_workbook = xlrd.open_workbook('data_sep.xls')
new_workbook = copy(old_workbook)
new_worksheet = new_workbook.get_sheet(5)  #Get the current sheet
data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[5]   #Open the current sheet
