...
table = data.sheets()[4]            #Open the next worksheet
...
new_worksheet = new_workbook.get_sheet(4)   #Open the current sheet
data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[3]       #Get the previous sheet
...
for i in range (0,543):
    if int(test_label[i])==result[i]:
        accu5+=1
acc_rate5=accu5/543
print("The prediction accuracy rate of the fifth month is ", acc_rate5)
...
old_workbook = xlrd.open_workbook('data_sep.xls')
new_workbook = copy(old_workbook)
new_worksheet = new_workbook.get_sheet(4)  #Get the current sheet
data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[4]   #Open the current sheet
