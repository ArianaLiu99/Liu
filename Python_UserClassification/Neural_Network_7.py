...
table = data.sheets()[6]            #Open the next worksheet
...
new_worksheet = new_workbook.get_sheet(6)   #Open the current sheet

data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[5]       #Get the previous sheet
...
for i in range (0,543):
    if int(test_label[i])==result[i]:
        accu7+=1
acc_rate7=accu7/543
print("The prediction accuracy rate of the seventh month is ", acc_rate7)
...
old_workbook = xlrd.open_workbook('data_sep.xls')
new_workbook = copy(old_workbook)
new_worksheet = new_workbook.get_sheet(6)  #Get the current sheet
data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[6]   #Open the current sheet
