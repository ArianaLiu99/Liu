...
table = data.sheets()[7]            #Open the next worksheet
...
new_worksheet = new_workbook.get_sheet(7)   #Open the current sheet

data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[6]       #Get the previous sheet
...
for i in range (0,543):
    if int(test_label[i])==result[i]:
        accu8+=1
acc_rate8=accu8/543
print("The prediction accuracy rate of the eighth month is ", acc_rate8)


#Adding result to the data
old_workbook = xlrd.open_workbook('data_sep.xls')
new_workbook = copy(old_workbook)
new_worksheet = new_workbook.get_sheet(7)  #Get the current sheet
data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[7]   #Open the current sheet
