...
table = data.sheets()[11]            #Open the next worksheet
...
new_worksheet = new_workbook.get_sheet(11)   #Open the current sheet

data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[10]       #Get the previous sheet
...
for i in range (0,543):
    if int(test_label[i])==result[i]:
        accu12+=1
acc_rate12=accu12/543
print("The prediction accuracy rate of the twelveth month is ", acc_rate12)


#Adding result to the data
old_workbook = xlrd.open_workbook('data_sep.xls')
new_workbook = copy(old_workbook)
new_worksheet = new_workbook.get_sheet(11)  #Get the current sheet
data=xlrd.open_workbook('data_sep.xls')
sheet1=data.sheets()[11]   #Open the current sheet
